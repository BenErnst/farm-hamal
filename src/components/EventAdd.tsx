...
const eventFormSchema = z.object({
  type: z.enum(["שריפה", "גניבה", "חיתוך גדר", "פרוטקשן", "פלישת עדר"]),
  ...
  useEffect(() => {
    if (farms.length) {
      const closestFarm = farms.reduce((prevFarm, currFarm) => {
        const prevDist =
          Math.abs(selectedLocation.lat - prevFarm?.location.lat) +
          Math.abs(selectedLocation.lng - prevFarm?.location.lng);
        const currDist =
          Math.abs(selectedLocation.lat - currFarm.location.lat) +
          Math.abs(selectedLocation.lng - currFarm.location.lng);
