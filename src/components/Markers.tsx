...
export const Markers = (props: Props) => {
  ...
  const handleDragEnd = useCallback(
    async (ev: google.maps.MapMouseEvent, entity: Farm | Event) => {
      if (map && ev.latLng) {
        ...
        const farmName =
          farms.find((f) => f.eventIds.includes((entity as Event).id))?.name ??
          "לא משויך";
        ToastService.showSuccessMsg(
          `מיקום האירוע עודכן: ${(entity as Event).type} | חווה: ${farmName}`,
        );
      }
    },
    [map, farms],
  ...
  return entities.map((entity) => (
    ...
    >
      ...
      >
        <div>
          {whichEntity === "farm" ? (
            <span style={{ fontSize: "18px" }}>{(entity as Farm).emoji}</span>
          ) : (
