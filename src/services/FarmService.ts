...
async function Update(farmToUpdate: Farm): Promise<Farm> {
  return await new Promise((resolve) => {
    ...
    const farmToUpdateIdx = farms.findIndex(
      (farm) => farm.id === farmToUpdate.id,
    );
    farms.splice(farmToUpdateIdx, 1, farmToUpdate);
    StorageService.save("farms", farms);
    const updatedFarm = farms.find(
      (farm) => farm.id === farmToUpdate.id,
    ) as Farm;
    resolve(updatedFarm);
  });
...
function getInitialFarms() {
  return [
    ...
    },
    {
      id: "f_2",
      name: "עגבניות מגידו",
      location: { lng: 35.18238, lat: 32.5766 },
    ...
    },
    {
      id: "f_3",
      name: "אבוקדו תענכים",
      location: { lng: 35.26893, lat: 32.5546 },
    ...
    },
    {
      id: "f_7",
      name: "מנגו הגלבוע",
      location: { lng: 35.3501, lat: 32.54023 },
      type: he.farmType.fieldCrops,
      farmer: {
        name: "עומר נחשון",
        phone: "054-*******",
